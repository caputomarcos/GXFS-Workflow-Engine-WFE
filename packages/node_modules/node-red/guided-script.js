...
waitForElm('.red-ui-deploy-button-content').then((elm) => {
    // Procura por todos os elementos span na página
    var aTags = document.querySelectorAll("span");
    var searchText = "Deploy";

    // Encontra o span com o texto "Deploy"
    var found = Array.from(aTags).find(tag => tag.textContent === searchText);

    // Se encontrou, muda o texto para "Save & Deploy"
    if (found) {
        found.innerHTML = 'Save & Deploy';
    }

    // Procura pelo elemento com o ID específico
    var deploy = document.getElementById("red-ui-header-button-deploy-icon");

    // Se o elemento existir, altera seu src com base no caminho da página
    if (deploy) {
        if (location.pathname === '/')
            deploy.src = '/red/pic/deploy.svg';
        else
            deploy.src = `${location.pathname}/red/pic/deploy.svg`;
    }
});

waitForElm('.red-ui-palette-node').then((elm) => {
    var logo = document.getElementById('red-ui-header');
    if (logo) {
        logo.style.display = "inline";
    }

    // var logoImage = document.querySelector(".red-ui-header-logo");
    // if (logoImage && logoImage.firstChild) {
    //     logoImage.firstChild.setAttribute("src", "red/pic/logo.png");
    // }
});
