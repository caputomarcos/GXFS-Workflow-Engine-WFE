    ...
    function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function (d, i) {
                ...
                if (d.type === "comment")
                    mainRect.classList.add("comment-node")
                if (d.type === "complete")
                    mainRect.classList.add("complete-node")
                if (d.type === "status")
                    mainRect.classList.add("status-node")
                if (d.type === "catch")
                    mainRect.classList.add("catch-node")
                //EFW       
                node[0][0].__mainRect__ = mainRect;
                d3.select(mainRect)
                    .on("mouseup", nodeMouseUp)
                ...
                // var hideLabel = d.hasOwnProperty('l') ? !d.l : false;
                var isHideLabel = d.hasOwnProperty('l') && !d.l      
                // console.log(`node.type: ${d.type}:${hideLabel}`)
                if (!isHideLabel){
                    if (color === "#DDAA99")
                ...
                // var isHideLabel = d.hasOwnProperty('l') ? !d.l : true;
                var inputLabel = d.inputLabels;
                if (isHideLabel || typeof inputLabel === "undefined") {
                    input.hidden = true; // innerHTML = "Input";
                    inputIcon.style.display = "none";
                    console.log(`node.input 1: ${d.type}:${hideLabel}:$${inputLabel}`)
                } else {
                    input.hidden = false
                    inputIcon.style.display = "block";
                    input.innerHTML = "Input " + d.inputLabels;
                    console.log(`node.input 2: ${d.type}:${hideLabel}:$${inputLabel}`)
                }
                //Node Output
                ...
                if (isHideLabel || typeof outputLabel === "undefined") {
                    output.hidden = true; //innerHTML = "Output";
                    outputIcon.style.display = "none";
                    console.log(`node.output 1: ${d.type}:${hideLabel}:$${inputLabel}`)
                } else {
                    output.hidden = false;
                    outputIcon.style.display = "block";
                    output.innerHTML = "Output " + d.outputLabels;
                    console.log(`node.output 2: ${d.type}:${hideLabel}:$${inputLabel}`)
                }
                //EFW
                node[0][0].appendChild(nodeContents);
                ...
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        ...
                        if (hideLabel || d.type === "link in" || d.type === "link out" || d.type === "link call") {
                            ...
                            d.h = Math.max(6 + 24 * labelParts.lines.length, (d.outputs || 0) * 15, 180);
                        } else if (d.inputLabels == undefined && d.outputLabels == undefined) {
                            d.h = Math.max(6 + 24 * labelParts.lines.length, (d.outputs || 0) * 15, 60);
                        } else {
                    ...
                    if (d.resize) {
                        ...
                        if (hideLabel || d.type === "link in" || d.type === "link out" || d.type === "link call") {
                            ...
                            d.w = Math.max(300, 20 * (Math.ceil((labelParts.width + 90 + (d._def.inputs > 0 ? 7 : 0)) / 20)));
                        } else {
